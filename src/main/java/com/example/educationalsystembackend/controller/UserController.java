...
public class UserController {
    ...
    public Result login(@Validated @RequestBody UserVO userVO) {
        ...
        LambdaQueryWrapper<UserEntity> userLambdaQueryWrapper = new LambdaQueryWrapper<UserEntity>().eq(UserEntity::getAccount, userVO.getAccount());
        ...
        if (!optional.isPresent() || !StringUtils.equals(userVO.getPassword(), user.getPassword())) //密码错误
            ...
            if (user.getAccess() == 1)
                ...
                LambdaQueryWrapper<StudentEntity> studentLambdaQueryWrapper = new LambdaQueryWrapper<StudentEntity>().eq(StudentEntity::getId, user.getAccount()).select(StudentEntity::getName);
                name = studentService.getOne(studentLambdaQueryWrapper).getName();
                map.put("name", name);
            } else {
                LambdaQueryWrapper<TeacherEntity> teacherLambdaQueryWrapper = new LambdaQueryWrapper<TeacherEntity>().eq(TeacherEntity::getId, user.getAccount()).select(TeacherEntity::getName);
    ...
    public Result sendCode(@NotBlank(message = "帐号不能为空") @PathVariable("account") String
                                   account, @Email(message = "邮箱格式错误") @PathVariable("email") String email) {
