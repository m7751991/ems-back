...
public interface TeacherMapper extends BaseMapper<TeacherEntity> {
    ...
    //添加老师
    @Insert("insert into teacher value (#{id},#{name},#{sex})")
    ...
    @Select("SELECT t.id,t.name AS teacher FROM teacher t,course c,choice ch WHERE ch.`course`=c.id AND c.`teacher`=t.id AND ch.`student`=#{student} GROUP BY t.name;")
    List<Map<String, Object>> queryStudentChoiceTeacher(String student);

    @Select("SELECT t.id,t.name AS teacher FROM choice ch,course c,teacher t WHERE c.id=ch.course and c.teacher=t.id AND c.id like #{course} and ch.student=#{student} group by t.name")
